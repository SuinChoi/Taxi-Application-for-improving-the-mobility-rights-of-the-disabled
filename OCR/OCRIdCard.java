...
public class OCRIdCard {
    ...
    private Rect cardRect; // 카드
    private Rect nameRect; //이름
    private Rect sexRect; // 성별
    private Rect ethnicityRect; // 국가
    private Rect yearRect; //년
    private Rect monthRect; // 월
    private Rect dayRect; // 일
    private Rect addressRect; // 주소
    private Rect numberRect; // 번호
    private Rect[] rects;
    ...
    public OCRIdCard(int widthPixel, int heightPixel, float ratio) {
        ...
        if ((float) widthPixel / heightPixel > (float) CardReferenceWidth / CardReferenceHeight) { // 높이에 초점
